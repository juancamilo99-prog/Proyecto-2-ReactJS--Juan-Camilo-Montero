...
const stats = [
    ...
    {value: '5', label: 'Viendo Actualmente', color: 'text-accent'},
    {value: '15+', label: 'Generos', color: 'text-primary'}
];
  ...
  return (
    <section className="hero-gradient text-center py-16 px-4">
        <p className="font-mono text-xs text-gray-500 neon-cyan uppercase tracking-widest mb-3">
            // descubre · explora · favorito · disfruta
        </p>
        ...
        <p className="text-muted-foreground text-base max-w-md mx-auto mb-6">
            Diferentes series y peliculas seleccionadas cuidadosamente. Descubre tu próxima obsesión.</p>
        <div className="flex items-center justify-center gap-3 flex-wrap">
            <button onClick={() => navigate('/explore')} 
            className="box-hero px-6 py-2.5 rounded-xl text-sm font-bold text-white">Explorar Todos los Animes</button>
            <button onClick={() => navigate('/favorites')}
                className="px-6 py-2.5 rounded-xl text-sm font-semibold border border-border text-muted-foreground
                hover:border-border hover:text-foreground transition-all">Mis favoritos ({favoritos.length})</button>
        </div>
